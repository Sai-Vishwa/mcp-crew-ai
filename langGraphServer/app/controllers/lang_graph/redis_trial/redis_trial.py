...
async def conn():
    async_client = await aget_client("redis://localhost:6379/0")
    
    sync_client = get_client("redis://localhost:6379/0")
    
    sync_client.set("meow" , "dass")
    
    print(sync_client.get("meow"))
    
    # val = await sync_client.get("meow")
    
    # print("await val is ==" , val)
    
    # val = async_client.get("meow")
    
    # print("async val ==> " , val)
    
    val = await async_client.get("meow")
    
    print("val in async await => ", val)
